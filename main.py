...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Main script for training RecSys-DiT.")
    parser.add_argument(
        "--config_path",
        type=str,
        default="configs/recsys_amazon.yaml",
        help="Path to the configuration file. Default is `configs/recsys_amazon.yaml`.",
    )
    parser.add_argument(
        "--deepspeed",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Pretrained text encoder name or path. Use "dummy" for offline smoke tests.',
    )
    parser.add_argument(
        "--pretrained_vision_encoder_name_or_path",
        type=str,
        default=None,
        help='Pretrained vision encoder name or path. Use "dummy" for offline smoke tests.',
    )
    parser.add_argument(
        "--buffer_root",
        type=str,
    ...
    parser.add_argument(
        ...
        default=0.1,
        help=(
            "The probability to randomly mask all conditioning modalities for a sample during training. "
            "If set to 0, the conditions are never globally masked."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="logs",
        help="Tracker log directory written under output_dir.",
    )
    parser.add_argument(
        "--allow_tf32",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default="tensorboard",
        choices=["tensorboard", "wandb", "comet_ml", "all", "none"],
        help=(
            'The integration to report the results and logs to. Supported platforms are `"tensorboard"`'
            ' (default), `"wandb"` and `"comet_ml"`. Use `"all"` to report to all integrations, '
            'or `"none"` to disable experiment trackers for smoke tests.'
        ),
    )
    parser.add_argument(
        "--sample_period",
    ...
    parser.add_argument(
        ...
        default="5,10,20",
        help=(
            "Comma-separated top-k list used to report retrieval metrics during "
            "`sample_period` on recommendation buffers. Set to an empty string to disable."
        ),
    )
    parser.add_argument(
        "--sample_similarity",
        ...
        help=(
            "Similarity used to retrieve items from `item_embeddings.npy` during "
            "training-time sampling evaluation."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Exclude valid history items from the retrieval candidate pool during "
            "training-time sampling evaluation."
        ),
    )
    parser.add_argument(
