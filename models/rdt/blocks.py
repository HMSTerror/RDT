...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor, 
        ...
        if mask is not None:
            ...
            if mask.shape != (B, L):
                raise ValueError(
                    f"Expected cross-attention mask with shape {(B, L)}, got {tuple(mask.shape)}."
                )

            # When an entire branch is masked out for a sample, prepend a zero token
            # and mark only that token as visible. This keeps attention numerically
            # stable while still making the branch contribute exactly zero.
            no_valid_tokens = ~mask.any(dim=1)
            if no_valid_tokens.any():
                null_token = torch.zeros(B, 1, C, device=c.device, dtype=c.dtype)
                c = torch.cat([null_token, c], dim=1)
                mask = torch.cat([no_valid_tokens.unsqueeze(1), mask], dim=1)
                L = c.shape[1]
...
class RDTBlock(nn.Module):
    """
    A RDT block with decoupled text/image cross-attention conditioning.
    """
    ...
    def forward(self, x, image_tokens, image_mask=None, text_tokens=None, text_mask=None):
        origin_x = x
        x = self.norm1(x)
        x = self.attn(x)
        x = x + origin_x
        ...
        origin_x = x
        x = self.norm2(x)
        x = self.text_cross_attn(x, text_tokens, text_mask)
        x = x + origin_x

        origin_x = x
        x = self.norm3(x)
        x = self.image_cross_attn(x, image_tokens, image_mask)
        x = x + origin_x
