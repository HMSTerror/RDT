...
from genrec.tokenization import build_layout_from_item_to_code, load_item_to_code  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate a GenRec DiT semantic-ID model on train/val/test tokenized buffers, "
            "including popularity-grouped metrics."
        )
    )
    parser.add_argument(
        "--config_path",
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Checkpoint directory or pytorch_model.bin file saved by train_genrec_dit.py.",
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Autocast precision for forward inference.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "`quantile_frequency` uses 33%%/66%% quantiles over train frequencies."
        ),
    )
    parser.add_argument(
...
def resolve_autocast_dtype(mixed_precision: str, device: torch.device) -> torch.dtype | None:
...
def load_indexed_item_codes(
    *,
    semantic_id_root: Path,
    buffer_root: Path,
    split_for_item_map: str,
) -> tuple[torch.Tensor, dict[int, str], dict[str, list[int]]]:
    ...
    item_to_code = load_item_to_code(semantic_id_root / "item_to_code.json")
    layout = build_layout_from_item_to_code(item_to_code)

    num_items = len(item_map)
    candidate_token_ids = np.zeros((num_items, layout.code_len), dtype=np.int64)
    for item_idx in range(num_items):
        item_id = idx_to_item[item_idx]
        if item_id not in item_to_code:
            raise KeyError(f"Item `{item_id}` from item_map.json is missing in item_to_code.json.")
        candidate_token_ids[item_idx] = np.asarray(layout.encode_codes(item_to_code[item_id]), dtype=np.int64)

    return torch.from_numpy(candidate_token_ids), idx_to_item, item_to_code
...
def extract_target_position_log_probs(
    ...
    row_outputs = []
    expected_codebooks = None
    for row_idx in range(batch_size):
        row_positions = torch.nonzero(labels[row_idx] != -100, as_tuple=False).flatten()
        if row_positions.numel() == 0:
            raise ValueError("Encountered a sample without masked target semantic positions.")
        row_codebook_ids = token_codebook_ids[row_idx, row_positions]
        order = torch.argsort(row_codebook_ids)
        row_positions = row_positions[order]
        row_codebook_ids = row_codebook_ids[order]
        if expected_codebooks is None:
            expected_codebooks = row_codebook_ids
        elif not torch.equal(expected_codebooks, row_codebook_ids):
            raise ValueError(
                "Inconsistent codebook ordering across the batch. "
                "All samples are expected to mask the same semantic codebook slots."
            )
        row_outputs.append(log_probs[row_idx, row_positions, :])
    return torch.stack(row_outputs, dim=0)


def score_all_candidate_items(
    *,
    ...
    batch = {
        key: value.to(model.token_embed.weight.device) if torch.is_tensor(value) else value
        for key, value in batch.items()
    }
    autocast_enabled = autocast_dtype is not None and model.token_embed.weight.device.type == "cuda"

    with torch.autocast(
        device_type=model.token_embed.weight.device.type,
    ...
    position_log_probs = extract_target_position_log_probs(
        log_probs,
        labels=batch["labels"],
        token_codebook_ids=batch["token_codebook_ids"],
    )

    batch_size = position_log_probs.shape[0]
    candidate_index = candidate_token_ids.t().unsqueeze(0).expand(batch_size, -1, -1).to(
        device=position_log_probs.device,
        dtype=torch.long,
    )
    score_parts = torch.gather(position_log_probs, dim=2, index=candidate_index)
    scores = score_parts.sum(dim=1)
    batch_loss = outputs.loss.detach().float() if outputs.loss is not None else torch.tensor(float("nan"))
    return scores, batch_loss
...
def plot_grouped_metrics(
    *,
    ...
    fig.suptitle(
        f"GenRec Grouped Evaluation on {split} (Cold / Mid / Hot)",
        fontsize=14,
        fontweight="bold",
    )
...
def main() -> None:
    ...
    autocast_dtype = resolve_autocast_dtype(args.mixed_precision, device)

    tokenized_root = Path(data_cfg["tokenized_root"])
    semantic_id_root = Path(data_cfg["semantic_id_root"])
    ...
    candidate_token_ids, idx_to_item, _ = load_indexed_item_codes(
        semantic_id_root=semantic_id_root,
        buffer_root=buffer_root,
        split_for_item_map=train_split,
    )
    candidate_token_ids = candidate_token_ids.to(device=device, dtype=torch.long)

    item_frequencies = load_split_target_frequencies(
        tokenized_root=tokenized_root,
        split=args.frequency_source_split,
        num_items=candidate_token_ids.shape[0],
    )
    ...
    progress_bar = tqdm(dataloader, total=total_batches, desc="GenRec Eval", unit="batch")
    ...
    for step, batch in enumerate(progress_bar, start=1):
        ...
        scores, batch_loss = score_all_candidate_items(
            model=model,
            batch=batch,
            candidate_token_ids=candidate_token_ids,
            autocast_dtype=autocast_dtype,
        )
        loss_values.append(float(batch_loss.item()))
        ...
        if args.save_jsonl is not None:
            ...
            for row_idx in range(target_item_ids.shape[0]):
                ...
                target_item = idx_to_item[target_item_id]
                group_name = GROUP_NAMES[int(batch_group_ids[row_idx].item())]
                predictions = []
                for rank_pos in range(max_k):
                    pred_item_id = int(topk_indices[row_idx, rank_pos].item())
                    predictions.append(
                        {
                            "rank": rank_pos + 1,
                            "item_id": pred_item_id,
                            "item_key": idx_to_item[pred_item_id],
                ...
                records.append(
                    {
                        ...
                        "target_item_key": target_item,
                        "target_frequency": int(item_frequencies[target_item_id]),
                        "target_group": group_name,
                        "target_group_zh": GROUP_NAMES_ZH[group_name],
        ...
        postfix = {
            "loss": f"{(sum(loss_values) / len(loss_values)):.4f}",
            "mean_rank": f"{running_metrics['mean_rank']:.2f}",
        }
        for k in topk_list:
    ...
    final_payload = {
        ...
        "semantic_id_root": str(semantic_id_root),
        "buffer_root": str(buffer_root),
        "evaluated_samples": int(overall_tracker.total),
        "candidate_items": int(candidate_token_ids.shape[0]),
        "topk": topk_list,
        "exclude_history_items": bool(args.exclude_history_items),
        "average_loss": float(sum(loss_values) / len(loss_values)) if loss_values else None,
