...
: "${OUTPUT_DIR:=checkpoints/genrec_hybrid_diffusion_amazon}"
...
# Enable full multimodal branch generation by default for the hybrid workflow.
: "${ENABLE_IMAGE:=1}"
: "${ENABLE_CF:=1}"
: "${ENABLE_FUSION:=1}"
: "${CF_METHOD:=item2vec}"
...
echo "========== [1/3] prepare multimodal embeddings + semantic IDs + tokenized splits =========="
bash scripts/prepare_genrec_semantic_ids.sh

echo "========== [2/3] train Hybrid GenRec Diffusion =========="
accelerate launch \
...
echo "========== [3/3] grouped test evaluation (Hybrid Diffusion) =========="
python scripts/eval_genrec_hybrid_diffusion.py \
  ...
  --save_json "${LOG_DIR}/genrec_hybrid_test_grouped_metrics.json" \
  --save_jsonl "${LOG_DIR}/genrec_hybrid_test_grouped_predictions.jsonl" \
  --save_plot "${LOG_DIR}/genrec_hybrid_test_grouped_metrics.png"

echo "========== hybrid diffusion pipeline finished =========="
